...
trait ReponseJson
{
    ...
    protected $logPath; //日志目录

    public function __construct()
    {
        $this->logPath = storage_path('logs/' . CLIENT . '/');
    }

    /**
     * return success json
     */
    public function success(string $message = '', array $data = [], $httpCode = 200, int $code = 200, string $jump = '', array $header = [])
    {
        $return = [
     ...
     * return failure json
     */
    public function failure(string $message = '', array $data = [], $httpCode = 200, int $code = 0, string $jump = '', array $header = [])
    {
        $return = [
            'status_code' => $httpCode,
